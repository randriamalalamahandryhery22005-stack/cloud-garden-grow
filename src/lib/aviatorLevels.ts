 ...
 * Déclenché sur un coefficient > 5.00x :
 *  - Résultat principal : +03:40 → +03:59, coefficient 2.00x → 5.00x (le plus fiable)
 *  - Indice : +04:40 → +04:59, coefficient 5.00x → 20.00x
 * Zone unique — aucune zone A/B.
 */
export function runLevel2(input: EngineInput): LevelOutcome {
  const { h, m, s, coefficient, stats } = input;
  const rng = new Rng(makeSeed(h, m, s, coefficient, 2, stats.volatility));
  const bias = biasFrom(stats);
  const now = new Date();

  const off1 = rng.int(220, 239); // 03:40 → 03:59
  const off2 = rng.int(280, 299); // 04:40 → 04:59
  const t1 = addSeconds(h, m, s, off1);
  const t2 = addSeconds(h, m, s, off2);

  const main = mapRange(rng.next(), 2.0, 5.0, bias);
  const index = mapRange(rng.next(), 5.0, 20.0, bias);

  const precisionBase = clamp(86 + stats.mid2to5Ratio * 8 - stats.volatility * 0.35, 70, 96);

  const mk = (kind: LevelResultRow["kind"], label: string, value: number, t: { h: number; m: number; s: number }, off: number): LevelResultRow => {
    const { confidence, reliability } = confidenceFor(rng, kind === "main" ? precisionBase : precisionBase - 10, value);
    return {
      kind,
      label,
      ...
      risk: riskOf(value),
      offsetLabel: `+${pad(Math.floor(off / 60))} min ${pad(off % 60)} s`,
    };
  };

  const rows = [
    mk("main", "Résultat principal", main, t1, off1),
    mk("index", "Indice haut", index, t2, off2),
  ];

  return buildOutcome(2, rows, precisionBase, stats, now, `${pad(h)}:${pad(m)}:${pad(s)}`, coefficient, "Dual Projection v2.0");
}

/**
 * NIVEAU 3 — Frappe haute.
 * Déclenché sur un coefficient > 5.00x :
 *  - Résultat principal : +02:00 → +02:15, coefficient 5.00x → 20.00x
 *  - Indice de protection : 2.00x → 5.00x (même fenêtre)
 * Zone unique — aucune zone A/B.
 */
export function runLevel3(input: EngineInput): LevelOutcome {
  const { h, m, s, coefficient, stats } = input;
  const rng = new Rng(makeSeed(h, m, s, coefficient, 3, stats.max));
  const bias = biasFrom(stats);
  const now = new Date();

  const off = rng.int(120, 135); // 02:00 → 02:15
  const t = addSeconds(h, m, s, off);
  ...
  const mk = (kind: LevelResultRow["kind"], label: string, value: number): LevelResultRow => {
  ...
  const rows = [
    mk("main", "Résultat principal", main),
    mk("protection", "Indice de protection", protect),
  ];

  return buildOutcome(3, rows, precisionBase, stats, now, `${pad(h)}:${pad(m)}:${pad(s)}`, coefficient, "High Strike v1.6");
}
